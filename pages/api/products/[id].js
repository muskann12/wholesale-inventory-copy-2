...
export default async function handler(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    // If variants changed (or any field that affects Shopify), sync full product
    ...
    if (variantsChanged || nameChanged || imageChanged) {
      ...
      // Legacy quantity sync (if you still support it) – but we now use variants
      // For variant‑only stock changes, we already handle in the frontend via the sales API.
      // However, if the admin changes stock directly in the product edit form, we need to sync each variant.
      if (updates.variants) {

...
export default function ProductPopup({ product, onClose, isMobile = false }) {
  ...
  const [showNegotiation, setShowNegotiation] = useState(false); // only used on mobile
  ...
  const handleSell = async () => {
    ...
    setSelling(true);
    // If negotiation is not shown (or not active), use the original selling price
    const finalPrice = (!isMobile || (showNegotiation && customPrice)) ? customPrice : null;
    const res = await fetch('/api/sales', {
      ...
      body: JSON.stringify({
        ...
        size: selectedSize,
        quantity,
        customSellingPrice: finalPrice && finalPrice !== product.variants.find(v => v.size === selectedSize)?.sellingPrice ? finalPrice : null,
      }),
    });
    if (res.ok) {
      alert('Sale recorded!');
  ...
  const costPrice = selectedVariant?.costPrice || 0;
  const originalMargin = selectedVariant ? ((selectedVariant.sellingPrice - costPrice) / selectedVariant.sellingPrice * 100).toFixed(1) : 0;
            ...
            <>
              ...
              <div className="p-5 border-b border-gray-100 grid grid-cols-2 gap-3">
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="text-base font-semibold">Rs {selectedVariant.sellingPrice.toLocaleString()}</p>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500">Stock</p>
                  <p className="text-base font-semibold">{selectedVariant.quantity} units</p>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500">Margin (MRP)</p>
                  <p className="text-base font-semibold text-green-600">{originalMargin}%</p>
                </div>
              </div>

              {/* Negotiation Section – only on mobile with toggle */}
              {isMobile && (
                <div className="p-5 border-b border-gray-100">
                  {!showNegotiation ? (
                    <button
                    ...
                    <div>
                      ...
                      />
                      {customPrice !== selectedVariant.sellingPrice && (
              ...
              {/* On desktop, always show negotiation field */}
              {!isMobile && (
                <div className="p-5 border-b border-gray-100">
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Negotiated Price (Rs)</label>
                  <input
